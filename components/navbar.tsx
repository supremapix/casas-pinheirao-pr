      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-xs font-bold uppercase tracking-widest transition-all relative group ${
                  pathname === link.path
                    ? "text-pinheirao-green"
                    : "text-pinheirao-black hover:text-pinheirao-green"
                } ${
                  link.highlight
                    ? "bg-pinheirao-green text-white px-4 py-2 rounded-full animate-pulse-slow shadow-lg hover:scale-105"
                    : ""
                }`}
              >
                {link.name}
                {!link.highlight && (
                  <span
                    className={`absolute bottom-[-4px] left-0 w-full h-0.5 bg-pinheirao-green transform transition-transform duration-300 ${
                      pathname === link.path
                        ? "scale-x-100"
          ...
          <div className="hidden lg:flex items-center space-x-6">
            <a
              href="tel:4136678015"
              className="flex items-center text-sm font-bold text-pinheirao-black hover:text-pinheirao-green"
            >
          ...
          <div className="lg:hidden">
            <button
              ...
              className="text-pinheirao-black"
            >
      ...
      >
        <div className="px-4 pt-2 pb-6 space-y-1">
          {navLinks.map((link) => (
            <Link
              ...
              className={`block px-3 py-3 text-xs sm:text-sm font-bold uppercase tracking-wider ${
                pathname === link.path
                  ? "text-pinheirao-green"
                  : "text-pinheirao-black border-b border-gray-100"
              }`}
            >
              {link.name}
            </Link>
          ...
          <div className="pt-4 grid grid-cols-1 gap-3">
            <a
              href="tel:4136678015"
              className="flex items-center justify-center py-3 text-sm bg-pinheirao-concrete rounded text-pinheirao-black font-bold"
            >
              <Phone size={18} className="mr-2" /> (41) 3667-8015
            </a>
            <a
              href="https://api.whatsapp.com/send?phone=5541996301028"
              className="flex items-center justify-center py-3 text-sm bg-pinheirao-green text-white rounded font-bold"
            >
